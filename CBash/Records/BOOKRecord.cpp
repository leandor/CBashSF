...
signed long BOOKRecord::ParseRecord(unsigned char *buffer, const unsigned long &recSize)
    {
    if(IsLoaded())
        return -1;
    IsLoaded(true);
...
unsigned long BOOKRecord::GetSize(bool forceCalc)
    {
...
signed long BOOKRecord::WriteRecord(_FileHandler &SaveHandler)
    {
    if(EDID.IsLoaded())
...
#ifdef _DEBUG
void BOOKRecord::Debug(int debugLevel)
    {
    if(!IsLoaded())
        return;
