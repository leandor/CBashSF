...
int ARMORecord::ParseRecord(unsigned char *buffer, const unsigned int &recSize)
    {
    if(IsLoaded())
        return -1;
    IsLoaded(true);
...
unsigned int ARMORecord::GetSize(bool forceCalc)
    {
...
int ARMORecord::WriteRecord(_FileHandler &SaveHandler)
    {
    if(EDID.IsLoaded())
