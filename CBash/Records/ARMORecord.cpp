...
signed long ARMORecord::ParseRecord(unsigned char *buffer, const unsigned long &recSize)
    {
    if(IsLoaded())
        return -1;
    IsLoaded(true);
...
unsigned long ARMORecord::GetSize(bool forceCalc)
    {
...
signed long ARMORecord::WriteRecord(_FileHandler &SaveHandler)
    {
    if(EDID.IsLoaded())
