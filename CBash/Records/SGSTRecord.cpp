...
#ifdef _DEBUG
void SGSTRecord::Debug(int debugLevel)
    {
    if(!IsLoaded())
        return;
    ...
    if(Effects.size())
        ...
        for(unsigned int p = 0;p < Effects.size();p++)
            ...
            PrintIndent(indentation);
            printf("Index: %u\n", p);
            Effects[p]->EFID.Debug(NULL, debugLevel, indentation);
            Effects[p]->EFIT.Debug(NULL, debugLevel, indentation);
            Effects[p]->SCIT.Debug(NULL, debugLevel, indentation);
            Effects[p]->FULL.Debug(NULL, debugLevel, indentation);
            }
        indentation -= 2;
        }
