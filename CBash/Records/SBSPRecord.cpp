...
signed long SBSPRecord::ParseRecord(unsigned char *buffer, const unsigned long &recSize)
    {
    if(IsLoaded())
        return -1;
    IsLoaded(true);
...
unsigned long SBSPRecord::GetSize(bool forceCalc)
    {
...
signed long SBSPRecord::WriteRecord(_FileHandler &SaveHandler)
    {
    if(EDID.IsLoaded())
...
#ifdef _DEBUG
void SBSPRecord::Debug(int debugLevel)
    {
    if(!IsLoaded())
        return;
