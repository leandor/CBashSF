...
signed long EFSHRecord::ParseRecord(unsigned char *buffer, const unsigned long &recSize)
    {
    if(IsLoaded())
        return -1;
    IsLoaded(true);
...
unsigned long EFSHRecord::GetSize(bool forceCalc)
    {
...
signed long EFSHRecord::WriteRecord(_FileHandler &SaveHandler)
    {
    if(EDID.IsLoaded())
...
#ifdef _DEBUG
void EFSHRecord::Debug(int debugLevel)
    {
    if(!IsLoaded())
        return;
