...
int GRASRecord::ParseRecord(unsigned char *buffer, const unsigned int &recSize)
    {
    if(IsLoaded())
        return -1;
    IsLoaded(true);
...
unsigned int GRASRecord::GetSize(bool forceCalc)
    {
...
int GRASRecord::WriteRecord(_FileHandler &SaveHandler)
    {
    if(EDID.IsLoaded())
