...
signed long GRASRecord::ParseRecord(unsigned char *buffer, const unsigned long &recSize)
    {
    if(IsLoaded())
        return -1;
    IsLoaded(true);
...
unsigned long GRASRecord::GetSize(bool forceCalc)
    {
...
signed long GRASRecord::WriteRecord(_FileHandler &SaveHandler)
    {
    if(EDID.IsLoaded())
...
#ifdef _DEBUG
void GRASRecord::Debug(int debugLevel)
    {
    if(!IsLoaded())
        return;
