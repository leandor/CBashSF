...
int ANIORecord::ParseRecord(unsigned char *buffer, const unsigned int &recSize)
    {
    if(IsLoaded())
        return -1;
    IsLoaded(true);
...
unsigned int ANIORecord::GetSize(bool forceCalc)
    {
...
int ANIORecord::WriteRecord(_FileHandler &SaveHandler)
    {
    if(EDID.IsLoaded())
