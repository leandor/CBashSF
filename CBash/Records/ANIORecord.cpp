...
signed long ANIORecord::ParseRecord(unsigned char *buffer, const unsigned long &recSize)
    {
    if(IsLoaded())
        return -1;
    IsLoaded(true);
...
unsigned long ANIORecord::GetSize(bool forceCalc)
    {
...
signed long ANIORecord::WriteRecord(_FileHandler &SaveHandler)
    {
    if(EDID.IsLoaded())
...
#ifdef _DEBUG
void ANIORecord::Debug(int debugLevel)
    {
    if(!IsLoaded())
        return;
