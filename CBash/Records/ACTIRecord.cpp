...
signed long ACTIRecord::ParseRecord(unsigned char *buffer, const unsigned long &recSize)
    {
    if(IsLoaded())
        return -1;
    IsLoaded(true);
...
unsigned long ACTIRecord::GetSize(bool forceCalc)
    {
...
signed long ACTIRecord::WriteRecord(_FileHandler &SaveHandler)
    {
    if(EDID.IsLoaded())
