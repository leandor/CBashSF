...
int SLGMRecord::ParseRecord(unsigned char *buffer, const unsigned int &recSize)
    ...
    while(curPos < recSize){
        ...
        switch(subType)
            ...
            default:
                //printf("FileName = %s\n", FileName);
                printf("  SLGM: Unknown subType = %04x\n", subType);
                printf("  Size = %i\n", subSize);
                printf("  CurPos = %04x\n\n", recStart + curPos - 6);
                curPos = recSize;
                break;
...
unsigned int SLGMRecord::GetSize()
    {
...
int SLGMRecord::WriteRecord(unsigned char *buffer, unsigned int &usedBuffer)
    ...
    if(SOUL.IsLoaded())
        _writeSubRecord(buffer, eSOUL, SOUL.GetSize(), &SOUL.value, usedBuffer);
    if(SLCP.IsLoaded())
        _writeSubRecord(buffer, eSLCP, SLCP.GetSize(), &SLCP.value, usedBuffer);
    return -1;
    }
