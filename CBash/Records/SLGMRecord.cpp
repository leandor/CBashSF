...
signed long SLGMRecord::ParseRecord(unsigned char *buffer, const unsigned long &recSize)
    {
    if(IsLoaded())
        return -1;
    IsLoaded(true);
...
unsigned long SLGMRecord::GetSize(bool forceCalc)
    {
...
signed long SLGMRecord::WriteRecord(_FileHandler &SaveHandler)
    {
    if(EDID.IsLoaded())
    ...
    if(SOUL.IsLoaded())
        SaveHandler.writeSubRecord(eSOUL, &SOUL.value, SOUL.GetSize());
    if(SLCP.IsLoaded())
        SaveHandler.writeSubRecord(eSLCP, &SLCP.value, SLCP.GetSize());
    return -1;
    }

#ifdef _DEBUG
void SLGMRecord::Debug(int debugLevel)
    {
    if(!IsLoaded())
        return;
    ...
    SOUL.Debug("SOUL", debugLevel, indentation);

    SLCP.Debug("SLCP", debugLevel, indentation);
    }
#endif
