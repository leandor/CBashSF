...
signed long INGRRecord::ParseRecord(unsigned char *buffer, const unsigned long &recSize)
    {
    if(IsLoaded())
        return -1;
    IsLoaded(true);
...
unsigned long INGRRecord::GetSize(bool forceCalc)
    {
...
signed long INGRRecord::WriteRecord(_FileHandler &SaveHandler)
    {
    if(EDID.IsLoaded())
...
#ifdef _DEBUG
void INGRRecord::Debug(int debugLevel)
    {
    if(!IsLoaded())
        return;
