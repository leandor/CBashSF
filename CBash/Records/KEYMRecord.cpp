...
signed long KEYMRecord::ParseRecord(unsigned char *buffer, const unsigned long &recSize)
    {
    if(IsLoaded())
        return -1;
    IsLoaded(true);
...
unsigned long KEYMRecord::GetSize(bool forceCalc)
    {
...
signed long KEYMRecord::WriteRecord(_FileHandler &SaveHandler)
    {
    if(EDID.IsLoaded())
...
#ifdef _DEBUG
void KEYMRecord::Debug(int debugLevel)
    {
    if(!IsLoaded())
        return;
