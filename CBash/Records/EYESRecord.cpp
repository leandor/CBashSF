...
int EYESRecord::ParseRecord(unsigned char *buffer, const unsigned int &recSize)
    {
    if(IsLoaded())
        return -1;
    IsLoaded(true);
...
unsigned int EYESRecord::GetSize(bool forceCalc)
    {
...
int EYESRecord::WriteRecord(_FileHandler &SaveHandler)
    {
    if(EDID.IsLoaded())
