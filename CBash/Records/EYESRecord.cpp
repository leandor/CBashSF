...
signed long EYESRecord::ParseRecord(unsigned char *buffer, const unsigned long &recSize)
    {
    if(IsLoaded())
        return -1;
    IsLoaded(true);
...
unsigned long EYESRecord::GetSize(bool forceCalc)
    {
...
signed long EYESRecord::WriteRecord(_FileHandler &SaveHandler)
    {
    if(EDID.IsLoaded())
...
#ifdef _DEBUG
void EYESRecord::Debug(int debugLevel)
    {
    if(!IsLoaded())
        return;
