...
int APPARecord::ParseRecord(unsigned char *buffer, const unsigned int &recSize)
    {
    if(IsLoaded())
        return -1;
    IsLoaded(true);
...
unsigned int APPARecord::GetSize(bool forceCalc)
    {
...
int APPARecord::WriteRecord(_FileHandler &SaveHandler)
    {
    if(EDID.IsLoaded())
