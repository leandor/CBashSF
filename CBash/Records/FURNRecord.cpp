...
signed long FURNRecord::ParseRecord(unsigned char *buffer, const unsigned long &recSize)
    {
    if(IsLoaded())
        return -1;
    IsLoaded(true);
...
unsigned long FURNRecord::GetSize(bool forceCalc)
    {
...
signed long FURNRecord::WriteRecord(_FileHandler &SaveHandler)
    {
    if(EDID.IsLoaded())
...
#ifdef _DEBUG
void FURNRecord::Debug(int debugLevel)
    {
    if(!IsLoaded())
        return;
