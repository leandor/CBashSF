...
signed long SKILRecord::ParseRecord(unsigned char *buffer, const unsigned long &recSize)
    {
    if(IsLoaded())
        return -1;
    IsLoaded(true);
...
unsigned long SKILRecord::GetSize(bool forceCalc)
    {
...
signed long SKILRecord::WriteRecord(_FileHandler &SaveHandler)
    {
    if(EDID.IsLoaded())
