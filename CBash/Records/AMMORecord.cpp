...
signed long AMMORecord::ParseRecord(unsigned char *buffer, const unsigned long &recSize)
    {
    if(IsLoaded())
        return -1;
    IsLoaded(true);
...
unsigned long AMMORecord::GetSize(bool forceCalc)
    {
...
signed long AMMORecord::WriteRecord(_FileHandler &SaveHandler)
    {
    if(EDID.IsLoaded())
...
#ifdef _DEBUG
void AMMORecord::Debug(int debugLevel)
    {
    if(!IsLoaded())
        return;
