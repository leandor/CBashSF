...
signed long DOORRecord::ParseRecord(unsigned char *buffer, const unsigned long &recSize)
    {
    if(IsLoaded())
        return -1;
    IsLoaded(true);
...
unsigned long DOORRecord::GetSize(bool forceCalc)
    {
    ...
    if(TNAM.size())
        TotSize += (unsigned long)TNAM.size() * (sizeof(unsigned long) + 6);
    return TotSize;
    }

signed long DOORRecord::WriteRecord(_FileHandler &SaveHandler)
    {
    if(EDID.IsLoaded())
    ...
    if(TNAM.size())
        for(unsigned long p = 0; p < TNAM.size(); p++)
            SaveHandler.writeSubRecord(eTNAM, TNAM[p], sizeof(unsigned long));
    return -1;
    }
