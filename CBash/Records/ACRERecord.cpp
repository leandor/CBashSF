...
signed long ACRERecord::ParseRecord(unsigned char *buffer, const unsigned long &recSize)
    {
    if(IsLoaded())
        return -1;
    IsLoaded(true);
...
unsigned long ACRERecord::GetSize(bool forceCalc)
    {
...
signed long ACRERecord::WriteRecord(_FileHandler &SaveHandler)
    {
    char null = 0;
