...
signed long LSCRRecord::ParseRecord(unsigned char *buffer, const unsigned long &recSize)
    {
    if(IsLoaded())
        return -1;
    IsLoaded(true);
...
unsigned long LSCRRecord::GetSize(bool forceCalc)
    {
    ...
    for(unsigned long p = 0; p < LNAM.size(); p++)
        if(LNAM[p]->IsLoaded())
            TotSize += LNAM[p]->GetSize() + 6;
    return TotSize;
    }

signed long LSCRRecord::WriteRecord(_FileHandler &SaveHandler)
    {
    if(EDID.IsLoaded())
