...
signed long STATRecord::ParseRecord(unsigned char *buffer, const unsigned long &recSize)
    {
    if(IsLoaded())
        return -1;
    IsLoaded(true);
...
unsigned long STATRecord::GetSize(bool forceCalc)
    {
...
signed long STATRecord::WriteRecord(_FileHandler &SaveHandler)
    {
    if(EDID.IsLoaded())
